...
func GetMyBooking(db *sql.DB, cid, passportNo, lang string) (*qancUtils.BookingDetail, error) {
	...
	if strings.TrimSpace(cid) != "" {
		row = db.QueryRow(`
			SELECT b.id, s.id, s.slot_date::text, b.queue_no, b.full_name, b.phone, b.status, b.created_at::text
			FROM anc_bookings b
	...
	if err := row.Scan(&b.ID, &b.SlotID, &b.SlotDate, &b.QueueNo, &b.FullName, &b.Phone, &b.Status, &b.CreatedAt); err != nil {
